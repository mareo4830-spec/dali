...
export default function PublicMenu() {
  ...
  // Estado de las bebidas en tiempo real desde Firestore (con fallback a MENU_ITEMS local)
    ...
    const unsubscribe = subscribeToMenuItems(
      (items) => {
        if (items && items.length > 0) {
          ...
          // Si Firestore está vacío inicialmente, mostramos los ítems por defecto
          setMenuItems(MENU_ITEMS);
        }
      },
      (error) => {
        console.warn('Usando catálogo local por incidencia de conexión:', error);
      }
    );
